...
(function () {
  ...
  var coat = window.setup.setupContainer.querySelector('.wizard-coat');

  // Инпут с цветом мантии персонажа игрока, который отправляет цвет на сервер
  var inputWizardCoat = window.setup.setupContainer.querySelector('input[name="coat-color"]');

  // Цвет глаз персонажа игрока
  var eyes = window.setup.setupContainer.querySelector('.wizard-eyes');

  // Инпут с цветом глаз персонажа игрока, который отправляет цвет на сервер
  var inputWizardEyes = window.setup.setupContainer.querySelector('input[name="eyes-color"]');

  // Цвет фаербола персонажа игрока
  var fireball = window.setup.setupContainer.querySelector('.setup-fireball-wrap');

  // Инпут с цветом фаербола персонажа игрока, который отправляет цвет на сервер
  var inputWizardFireball = window.setup.setupContainer.querySelector('input[name="fireball-color"]');
