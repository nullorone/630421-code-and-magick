...
var renderStatistics = function(ctx,names, times) {
  var canvas = document.querySelector('#canvas-stat');
  ...
  if (ctx) {
    ...
    for (var i = 0; i < names.length; i++) {
      ...
      ctx.fillText(names[i], RESULT_POSITION_X + i * (GAP_BAR + WIDTH_BAR), PLAYER_NAME_POSITION_Y);//Отрисовка имени игрока. На каждой итерации смещает начальную позицию на 90px по-горизонтали
      ...
      ctx.translate((RESULT_POSITION_X + WIDTH_BAR) + i * (GAP_BAR + WIDTH_BAR), PLAYER_BAR_POSITION_Y); //Переносит систему координат в начало отрисовки столбца
      ...
      ctx.fillStyle = '#000000';
      ctx.fillText(Math.floor(times[i]), TEXT_TIMES_POSITION_X, textTimesPositionY);//Отрисовывает результат игрока с заданным положением относительно столбца
